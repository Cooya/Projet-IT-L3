...
void ajouter_etat_final_bis(intptr_t element, void *data) {
	Automate *automate = (Automate *)data;

	ajouter_etat_final(automate, element);
}

void ajouter_etat_initial_bis(intptr_t element, void *data) {
	Automate *automate = (Automate *)data;

	ajouter_etat_initial(automate, element);
}

Automate *miroir( const Automate * automate){
	Automate *automate_miroir = creer_automate();

	pour_toute_transition( // Pour toute transition de l'automate de base on ajoute la transition inverse à notre nouvel automate.
		automate,
		ajouter_transition_inverse,
		automate_miroir
	);

	pour_tout_element(
		automate->initiaux,
		ajouter_etat_final_bis,
		automate_miroir
	);

	pour_tout_element(
		automate->finaux,
		ajouter_etat_initial_bis,
		automate_miroir
	);
...
Automate * creer_automate_minimal( const Automate* automate ){
	Automate *etape1 = miroir(automate);

	Automate *etape2 = creer_automate_deterministe(etape1);
	liberer_automate(etape1);

	Automate *etape3 = miroir(etape2);
	liberer_automate(etape2);

	Automate *etape4 = creer_automate_deterministe(etape3);
	liberer_automate(etape3);
